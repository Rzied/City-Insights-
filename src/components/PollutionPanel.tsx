  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-100 bg-white/60 p-4">
        <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Indice global</p>
        <p className="mt-2 font-display text-3xl font-semibold text-ink">{data.aqi}</p>
        <p className="text-sm text-slate-600">{data.category} · Dominant {data.dominantPollutant}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-2">
        {data.measurements.map((measurement) => (
          <div
            key={measurement.label}
            className="rounded-2xl border border-slate-100 bg-white/60 p-3 text-sm text-slate-700"
          >
